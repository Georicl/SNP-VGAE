"""
VGAE 基因组预测（无 VAE 模式）— 推理 + SNP 归因 + 结果输出
==========================================================
端到端流程（跳过 SNP-VAE 预训练，直接使用原始基因型）:
  1. 训练 VGAENoVAE (5-fold CV, 保留每折模型)
  2. 表型预测 (跨折聚合)
  3. SNP 归因 (直接梯度, 跨折聚合)
  4. 显著性评估 (置换 null 分布 + Bonferroni)
  5. 输出标准结果文件

与 predict_and_attribute.py 的区别:
  - 无 SNP-VAE 预训练步骤
  - 节点特征 = 插补后的原始基因型 (N, M)
  - 模型为 VGAENoVAE (9728 → 512 → 128 → d_z 渐进压缩)
  - 归因使用 compute_snp_scores_raw (X = G，无需嵌入反投影)

输出文件 (test/result/<trait>_no_vae/):
...
运行:
  PYTHONPATH=pytorch/src uv run python test/predict_and_attribute_no_vae.py
"""
...
from model.attribution import (
    compute_snp_scores_raw, top_snps, aggregate_cv_scores,
)
...
# 模型配置 (无 VAE；d_snp 运行时从基因型自动检测)
VGAE_CONFIG: dict[str, int | float] = dict(
    k_neighbors=30, d_hidden1=512, d_hidden2=128, d_z=16,
    dropout=0.7, mlp_hidden=64, lr=1e-3, alpha=0.1, beta_kl=0.01,
    max_epochs=200, patience=30, weight_decay=1e-4, kl_warmup_epochs=30,
)
...
def make_model(d_snp):
    """按配置创建 VGAENoVAE 模型。"""
    return VGAENoVAE(
        d_snp=d_snp, d_hidden1=int(VGAE_CONFIG["d_hidden1"]),
        d_hidden2=int(VGAE_CONFIG["d_hidden2"]), d_z=int(VGAE_CONFIG["d_z"]),
        dropout=float(VGAE_CONFIG["dropout"]), mlp_hidden=int(VGAE_CONFIG["mlp_hidden"]),
    )


def train_and_attribute(genotype, grm, labels, device):
    """训练 5-fold VGAENoVAE 并计算跨折 SNP 归因。"""
    N = len(labels)
    M = genotype.shape[0]
    masks = generate_kfold_masks(N, k_folds=5, seed=42)

    builder = GraphBuilder()
    adj_norm, adj_raw = builder.knn_graph_builder(
        grm, k=int(VGAE_CONFIG["k_neighbors"]))
    node_features = builder.build_node_features(genotype)  # 无 VAE 嵌入
    ...
    for fold_idx, (train_mask, val_mask, test_mask) in enumerate(masks):
        ...
        trainer = VGAETrainer(
            ...
            lr=float(VGAE_CONFIG["lr"]), alpha=float(VGAE_CONFIG["alpha"]),
            beta=float(VGAE_CONFIG["beta_kl"]), weight_decay=float(VGAE_CONFIG["weight_decay"]),
            kl_warmup_epochs=int(VGAE_CONFIG["kl_warmup_epochs"]),
            patience=int(VGAE_CONFIG["patience"]), max_epochs=int(VGAE_CONFIG["max_epochs"]),
        ...
        # SNP 归因 (原始基因型模式: X = G，直接梯度)
        print(f"  计算 SNP 归因...")
        scores = compute_snp_scores_raw(
            model=model, node_features=node_features, adj_norm=adj_norm,
            genotype=genotype, aggregate="sum", device=device,
        )
...
def compute_attribution_null(genotype, grm, labels, adj_norm, node_features,
                             n_perm, device):
    """通过置换表型计算归因 null 分布。"""
    ...
    for i in range(n_perm):
        ...
        trainer = VGAETrainer(
            ...
            lr=1e-3, alpha=0.1, beta=0.01, weight_decay=1e-4,
...
def main():
    ...
    print("=" * 70)
    print(f"VGAENoVAE 推理 + SNP 归因（无 VAE）— {TRAIT_NAME}")
    print("=" * 70)
    ...
    # 训练 + 归因（无 VAE 预训练，直接跳过）
    print("\n[2] VGAENoVAE 5-fold 训练 + SNP 归因...")
    t0 = time.time()
    y_pred, snp_scores, fold_models = train_and_attribute(
        geno_sub, grm_sub, lab_sub, device
    )
    ...
    # 显著性评估
    print(f"\n[3] 归因显著性 (置换 null, n={N_PERM_ATTRIB})...")
    builder = GraphBuilder()
    adj_norm, _ = builder.knn_graph_builder(
        grm_sub, k=int(VGAE_CONFIG["k_neighbors"]))
    node_features = builder.build_node_features(geno_sub)

    t0 = time.time()
    null_max = compute_attribution_null(
        geno_sub, grm_sub, lab_sub,
    ...
    # Bonferroni 校正阈值
    bonf_threshold = np.percentile(
        null_max, 100 * (1 - BONFERRONI_ALPHA / N_PERM_ATTRIB))
    print(f"  Bonferroni 阈值 (α={BONFERRONI_ALPHA}): {bonf_threshold:.4f}")

    # 输出结果
    print(f"\n[4] 输出结果...")
    out_dir = os.path.join(RESULT_DIR, f"{TRAIT_NAME}_no_vae")
    ...
    with open(attr_path, "w") as f:
        f.write(
            "SNP_IDX\tCHR\tSNP_NAME\tBP\tScore\tAbs_Score\tRank\tP_perm\tP_bonf\tSignificant\n")
        for j in range(M):
            info = snp_info[j] if j < len(snp_info) else {
                "chr": "?", "name": f"SNP_{j}", "bp": 0}
    ...
    with open(top_path, "w") as f:
        ...
        for rank_i, (idx_j, sc) in enumerate(
            ...
            info = snp_info[idx_j] if idx_j < len(
                snp_info) else {"chr": "?", "name": f"SNP_{idx_j}", "bp": 0}
